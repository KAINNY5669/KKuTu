...
function process(req, accessToken, MainDB, $p, done) {
    ...
    req.session.admin = GLOBAL.ADMIN.includes($p.id);
		...
		MainDB.users.findOne([ '_id', $p.id ]).on(($body) => {
			if($body) if($body.kkutu && $body.kkutu.hasOwnProperty('nickname') && ($body.kkutu.nickname != "" && $body.kkutu.nickname != null && $body.kkutu.nickname != undefined)) $p.name = $p.title = $body.kkutu.nickname;
