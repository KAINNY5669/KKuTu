...
function applyOptions(opt, isFirst){
	...
	var beforeBGM = $data.BGMTheme || 'kkutu';
	$data.BGMTheme = $data.opts.bt || 'kkutu';
	var changedBGM = $data.BGMTheme !== beforeBGM; 
	...
	/*if($data.bgm){
		$("#vol-bgm").val($data.muteBGM);
		$("#vol-eff").val($data.muteEFF);
	}*/
	if($data.bgm){
		$data.bgm.stop();
		$data.bgm = playBGM('lobby');
	}
}
function checkInput(){
...
function playSound(key, loop){
	...
	//var volume = (loop ? $data.muteBGM : $data.muteEFF)
